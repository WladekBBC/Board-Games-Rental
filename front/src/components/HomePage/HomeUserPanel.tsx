...
export default function HomeUserPanel() {
  ...
  const gamesWithAvailability = games.map((game) => {
    const rentedCount = rentals.filter(
      (r) => r.game.id === game.id && !r.returnedAt
    ).length;
   ...
   **/
  const availableGames = gamesWithAvailability.filter(
    (game) => game.availableQuantity > 0
  );
  const totalAvailableCopies = gamesWithAvailability.reduce(
    (total, game) => total + game.availableQuantity,
    0
  );
  ...
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
        <Link
          href={[Perms.A, Perms.R].includes(permissions) ? "/games" : "/"}
        ...
        <Link
          href={[Perms.A, Perms.R].includes(permissions) ? "/rentals" : "/"}
          className="block h-full"
        >
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow h-full flex flex-col">
            <h2 className="text-2xl font-bold mb-2 text-gray-800 dark:text-white">
        ...
        {user && (
          <Link
            href={permissions == Perms.A ? "/users" : "/"}
            className="block h-full"
          >
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow h-full flex flex-col">
              <h2 className="text-2xl font-bold mb-2 text-gray-800 dark:text-white">
